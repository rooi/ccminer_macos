...
__device__ __host__ void fp_zero( fp_int *a )
{
	for( int i = 0; i < FP_SIZE; i++ ) a->dp[i] = 0;
	a->used = 0;
}

__device__ __host__ void fp_set( fp_int *a, fp_digit b )
{
	fp_zero(a);
	a->dp[0] = b;
	a->used  = a->dp[0] ? 1 : 0;
...
__device__ __host__ int fp_cmp_mag( fp_int *a, fp_int *b )
{
...
/* unsigned addition */
__device__ __host__ void s_fp_add(fp_int *a, fp_int *b, fp_int *c)
{
  int      x, y, oldused;
  fp_word  t;
...
__device__ __host__ void s_fp_sub(fp_int *a, fp_int *b, fp_int *c)
{
...
__device__ __host__ void fp_add( fp_int *a, fp_int *b, fp_int *c )
{
	s_fp_add (a, b, c);
}

/* c = a - b */
__device__ __host__ void fp_sub(fp_int *a, fp_int *b, fp_int *c)
{
//  int     sa, sb;
...
__device__ __host__ int fp_cmp(fp_int *a, fp_int *b)
{
	 return fp_cmp_mag(a, b);
}

__device__ __host__ void fp_copy( fp_int *a, fp_int *b )
{
	for( int i = 0; i < FP_SIZE; i++ ) b->dp[i] = a->dp[i];
	b->used = a->used;
}

// TODO: use clz, return a->used - clz(a->dp[a->used-1])
__device__ __host__ int fp_count_bits( fp_int *a )
{
	int     r;
	fp_digit q;
...
__device__ __host__ void fp_rshd( fp_int *a, int x )
{
	int y;
...
__device__ __host__ void fp_lshd(fp_int *a, int x)
{
	int y;
...
/* c = a * 2**d */
__device__ __host__ void fp_mul_2d( fp_int *a, int b, fp_int *c )
{
...
/* c = a * b */
__device__ __host__ void fp_mul_d( fp_int *a, fp_digit b, fp_int *c )
{
	fp_word  w;
	int      x, oldused;
...
/* c = a mod 2**d */
__device__ __host__ void fp_mod_2d( fp_int *a, int b, fp_int *c )
{
	int x;
...
/* c = a / 2**b */
__device__ __host__ void fp_div_2d( fp_int *a, int b, fp_int *c, fp_int *d )
{
	fp_digit D, r, rr;
	int      x;
	fp_int   t;
...
/* a/b => cb + d == a */
__device__ __host__ void fp_div( fp_int *a, fp_int *b, fp_int *c, fp_int *d )
{
	fp_int q, x, y, t1, t2;
	int n, t, i, norm;
...
__device__ __host__ void fp_2expt( fp_int *a, int b )
{
	int z;
...
__device__ __host__ void fp_mul_comba(fp_int *A, fp_int *B, fp_int *C)
{
...
__device__ __host__ void fp_read_unsigned_bin(fp_int *a, unsigned char *b, int c)
{
	/* zero the int */
	fp_zero (a);
...
__device__ __host__ void fp_reverse( unsigned char *s, int len )
{
  int     ix, iy;
  unsigned char t;
...
__device__ __host__ void fp_to_unsigned_bin( fp_int *a, unsigned char *b )
{
	int     x;
	fp_int  t;
...
__device__ __host__ void fp_reduce_setup( fp_int *a, fp_int *b )
{
	fp_2expt (a, b->used << 6);
	fp_div (a, b, a, NULL);
}
...
__device__ __host__ int fp_cmp_d( fp_int *a, fp_digit b )
{
...
__device__ __host__ void fp_reduce( fp_int *x, fp_int *m, fp_int *mu )
{
	fp_int q;
	int um = m->used;
	...
	fp_mul(&q, m, &q);
	q.used = um + 1;
	for( int i = q.used; i < FP_SIZE; i++ ) q.dp[i] = 0;

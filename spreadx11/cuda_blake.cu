...
__device__ __forceinline__ uint64_t SWAP64( const uint64_t x)
{
	uint64_t temp[2];
...
__host__ uint64_t host_SWAP64( uint64_t x )
{
		return
...
__device__ __forceinline__ uint64_t ROTR64_32( uint64_t val )
{
		asm("{\n\t"
		".reg .u32 tl,th; \n\t"
		"mov.b64 {tl,th},%1; \n\t"
...
__device__ __forceinline__ uint64_t ROTR64_16( uint64_t val )
{
		asm("{\n\t"
		".reg .u16 b0, b1, b2, b3; \n\t"
...
__device__ void blake_compress( uint64_t *h, const uint64_t *m, const int bits )
{
		uint64_t v[16];
...
__global__ void blake_gpu_hash_185(uint32_t threads, uint32_t startNonce, uint32_t *outputHash, uint32_t *g_signature, uint32_t *g_hashwholeblock)
{
...
#pragma unroll 8
		for (int i=0; i < 8; ++i) {
			outHash[2*i+0] = cuda_swap32( HIWORD(h[i]) );
			outHash[2*i+1] = cuda_swap32( LOWORD(h[i]) );
		}
	}
}

__host__
void blake_cpu_init(int thr_id, uint32_t threads)
{
...
__host__
void blake_cpu_hash_185(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *d_outputHash, uint32_t *d_signature, uint32_t *d_hashwholeblock)
{

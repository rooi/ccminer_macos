...
__constant__ uint32_t d_privkey_data[8]; // 32 bytes
__constant__ uint32_t d_kinv_data[8];    // 32 bytes
...
__global__
void spreadx11_sign_gpu(uint32_t threads, uint32_t startNonce, uint32_t *g_sha256hash, uint32_t *g_signature )
{
...
__host__
void spreadx11_sign_cpu_init(int thr_id, uint32_t throughput)
{
}
...
void spreadx11_sign_cpu_setInput(struct work *work)
{
	...
	fp_read_unsigned_bin(&fp_privkey, swork->privkey, 32);
	fp_read_unsigned_bin(&fp_kinv, swork->kinv, 32);

	cudaMemcpyToSymbol(d_privkey_data, fp_privkey.dp, 32, 0, cudaMemcpyHostToDevice);
	cudaMemcpyToSymbol(d_kinv_data, fp_kinv.dp, 32, 0, cudaMemcpyHostToDevice);
}

__host__
void spreadx11_sign_cpu(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *d_hash, uint32_t *d_signature)
{

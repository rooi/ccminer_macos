...
__global__
void spreadx11_sha256_gpu_hash_wholeblock(uint32_t threads, uint32_t startNonce, uint32_t *g_input, uint32_t *g_hash, uint32_t *g_signature)
{
...
__global__
void spreadx11_sha256double_gpu_hash_88(uint32_t threads, uint32_t startNonce, uint32_t *g_hash)
{
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
...
__host__
void spreadx11_sha256_cpu_init(int thr_id, uint32_t threads)
{
...
__host__
void spreadx11_sha256double_cpu_hash_88(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *d_hash)
{
...
*/
	...
	memcpy(paddedmessage+0, swork->data+84, 4); // nNonce
	memcpy(paddedmessage+4, swork->data+68, 8); // nTime
	memcpy(paddedmessage+12, swork->data+120, 65); // MinerSignature
	memcpy(paddedmessage+77, swork->data+0, 4); // nVersion
	memcpy(paddedmessage+81, swork->data+4, 32); // hashPrevBlock
	memcpy(paddedmessage+113, swork->data+36, 32); // HashMerkleRoot
	memcpy(paddedmessage+145, swork->data+76, 4); // nBits
	memcpy(paddedmessage+149, swork->data+80, 4); // nHeight
	memcpy(paddedmessage+153, swork->tx, swork->txsize); // tx
	...
	memcpy(pFillFooter, swork->data+4, (pFillEnd - pFillFooter)*4);
...
__host__
void spreadx11_sha256_cpu_hash_wholeblock(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *d_hash, uint32_t *d_signature, uint32_t *d_wholeblock)
{
	const uint32_t threadsperblock = 256;

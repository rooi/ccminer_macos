/* bitcoin algo (by klaus) */

#include "miner.h"
#include "cuda_helper.h"

static __thread uint32_t *h_nounce;
...
int scanhash_bitcoin(int thr_id, uint32_t *pdata, uint32_t *ptarget,
	uint32_t max_nonce, unsigned long *hashes_done)
{
	const uint32_t first_nonce = pdata[19];
	uint32_t throughput = device_intensity(thr_id, __func__, 1U << 25);

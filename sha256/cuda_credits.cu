...
static __device__ __forceinline__
void sha2_step1(const uint32_t a, const uint32_t b, const uint32_t c, uint32_t &d, const uint32_t e,
...
static __device__ __forceinline__
void sha2_step2(const uint32_t a, const uint32_t b, const uint32_t c, uint32_t &d, const uint32_t e,
...
void bitcredit_gpu_hash(int threads, uint32_t startNonce,uint32_t* NonceVector)
{
	...
	if (MAKE_ULONGLONG(cuda_swab32(state2.s6), cuda_swab32(state2.s7)) <= ((uint64_t*)pTarget)[3])
		NonceVector[0] = nonce;
}
...
__host__
void bitcredit_setBlockTarget(uint32_t* pdata, const uint32_t* zmidstate,const void *target)
{
	...
	((uint32_t*)PaddedMessage)[47] = 0x00000540;

	CUDA_SAFE_CALL(cudaMemcpyToSymbol(midstate, zmidstate, 32, 0, cudaMemcpyHostToDevice));
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(pTarget, target, 32, 0, cudaMemcpyHostToDevice));
	CUDA_SAFE_CALL(cudaMemcpyToSymbol(c_data, PaddedMessage, 48 * sizeof(uint32_t), 0, cudaMemcpyHostToDevice));
}

__host__
uint32_t bitcredit_cpu_hash(uint32_t thr_id, int threads, uint32_t startNounce,  int order)
{
	uint32_t result[MAX_GPUS];
	const int threadsperblock = 256;

	memset(result, 0xff, sizeof(result));
	cudaMemset(d_BitNonce[thr_id], 0xff, sizeof(uint32_t));
